...
class SportServiceTest {
    ...
    @BeforeEach
    void setup(){
        service = new SportService(repository);
    }
    @Test
...
//        assertEquals(exception.getMessage(), "No value present");
    ...
    void testSuccessfulSave() {
        var captor = ArgumentCaptor.forClass(SportStub.class);
        var role = SportStub.getSport();
        Mockito.when(repository.save(Mockito.any())).thenReturn(SportStub.getSport());
        var result = service.create(SportStub.getSport());
    ...
    void successfulGetAll() {
        var expect = SportStub.getAll();

        Mockito.when(repository.findAll(PageRequest.of(SportStub.page, SportStub.size)))
